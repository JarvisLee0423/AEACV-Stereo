...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root="/mnt/bn/depth-data-bn/SceneFlow", dstype="frames_finalpass", things_test=False):
...
class CREStereo(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/bn/depth-data-bn/CRES_data'):
        super(CREStereo, self).__init__(aug_params, reader=frame_utils.readDispCREStereo)

        raw_data_list = open(os.path.join(root, 'CRES_datalist.txt')).readlines()
        for raw_data_info in raw_data_list:
            raw_data_path = raw_data_info.split(',')
            self.image_list += [[raw_data_path[0], raw_data_path[1]]]
            self.disparity_list += [raw_data_path[2]]

        # image1_list = sorted(glob(root + '/**/*_left.jpg'))
        # image2_list = sorted(glob(root + '/**/*_right.jpg'))
        # disp_list = sorted(glob(root + '/**/*_left.disp.png'))
        # assert len(image1_list) == len(image2_list) == len(disp_list)
        # num_sample = len(image1_list)
...
class SintelStereo(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/bn/depth-data-bn/MPI_Stereo'):
        super(SintelStereo, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispSintelStereo)
...
class FallingThings(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/bn/depth-data-bn/fat'):
        super(FallingThings, self).__init__(aug_params, reader=frame_utils.readDispFallingThings)
...
class TartanAir(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/bn/depth-data-bn/stereo_data/Tartan_air', keywords=[]):
        super(TartanAir, self).__init__(aug_params, reader=frame_utils.readDispTartanAir)
        assert os.path.exists(root)
        ...
        image1_list = sorted(glob(root + '/*/*/*/*/image_left/*.png'))
        image2_list = sorted(glob(root + '/*/*/*/*/image_right/*.png'))
        disp_list = sorted(glob(root + '/*/*/*/*/depth_left/*.npy'))
...
class CARLA(StereoDataset):
    def __init__(self, aug_params=None, 
                 data_dir='/mnt/bn/depth-data-bn/HR-VS-Stereo/carla-highres/trainingF', # /mnt/bn/depth-data-bn/HR-VS-Stereo/
                 ):
        super(CARLA, self).__init__(aug_params)
                                    # half_resolution=True

        image1_list = sorted(glob(data_dir + '/*/im0.png'))
        image2_list = sorted(glob(data_dir + '/*/im1.png'))
        disp_list = sorted(glob(data_dir + '/*/disp0GT.pfm'))

        assert len(image1_list) == len(image2_list) == len(disp_list) > 0, [len(image1_list), len(image2_list), len(disp_list)]
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]
...
class InStereo2K(StereoDataset):
    def __init__(self, aug_params=None, 
                 data_dir='/mnt/bn/depth-data-bn/InStereo2K', # /mnt/bn/depth-data-bn/InStereo2K
                 ):
        super(InStereo2K, self).__init__(aug_params, reader=frame_utils.readDispInStereo2K)

        # merge train and test
        image1_list = sorted(glob(data_dir + '/train/*/*/left.png') + glob(data_dir + '/test/*/left.png'))
        image2_list = sorted(glob(data_dir + '/train/*/*/right.png') + glob(data_dir + '/test/*/right.png'))
        disp_list = sorted(glob(data_dir + '/train/*/*/left_disp.png') + glob(data_dir + '/test/*/left_disp.png'))
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        ...
        elif dataset_name == 'sceneflow_mixdata':
            clean_dataset = SceneFlowDatasets(aug_params, dstype='frames_cleanpass')
            final_dataset = SceneFlowDatasets(aug_params, dstype='frames_finalpass')
            new_dataset = (clean_dataset * 3) + (final_dataset * 3)
            logging.info(f"Adding {len(new_dataset)} samples from SceneFlow")
        elif dataset_name == 'crestereo':
            new_dataset = CREStereo(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from CRE Stereo")
        elif dataset_name == 'tartanair':
            new_dataset = TartanAir(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from Tartan Air")
        elif dataset_name == 'fallingthings':
            new_dataset = FallingThings(aug_params) * 3
            logging.info(f"Adding {len(new_dataset)} samples from Falling Things")
        elif dataset_name == 'sintelstereo':
            new_dataset = SintelStereo(aug_params) * 10
            logging.info(f"Adding {len(new_dataset)} samples from Sintel Stereo")
        elif dataset_name == 'hrvs':
            new_dataset = CARLA(aug_params) * 25
            logging.info(f"Adding {len(new_dataset)} samples from HR-VS-Stereo")
        elif dataset_name == 'instereo2k':
            new_dataset = InStereo2K(aug_params) * 10
