...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args):
        ...
        gev_raft_cor_planes = args.corr_levels * (2*args.corr_radius + 1) * (8+1)
        error_aware_cor_planes = args.error_aware_corr_windows * args.error_aware_corr_planes * args.error_aware_corr_groups
        
        self.convc1_1 = nn.Conv2d(gev_raft_cor_planes, 64, 1, padding=0)
        self.convc1_2 = nn.Conv2d(error_aware_cor_planes, 64, 1, padding=0)
    ...
    def forward(self, disp, corr):
        gev_raft_corr = corr[0]
        error_aware_corr = corr[1]
        
        gev_raft_cor = F.relu(self.convc1_1(gev_raft_corr))
        error_aware_cor = F.relu(self.convc1_2(error_aware_corr))
        cor = F.relu(self.convc2(gev_raft_cor + error_aware_cor))
