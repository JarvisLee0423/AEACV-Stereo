...
class MultiBasicEncoder(nn.Module):
    ...
    def forward(self, x, dual_inp=False, num_layers=3):
        ...
        if num_layers == 1:
            return (outputs04, v, low_f) if dual_inp else (outputs04, low_f)

        y = self.layer4(x)
        outputs08 = [f(y) for f in self.outputs08]

        if num_layers == 2:
            return (outputs04, outputs08, v, low_f) if dual_inp else (outputs04, outputs08, low_f)

        z = self.layer5(y)
        outputs16 = [f(z) for f in self.outputs16]

        return (outputs04, outputs08, outputs16, v, low_f) if dual_inp else (outputs04, outputs08, outputs16, low_f)
