...
@torch.no_grad()
def validate_eth3d(model, logdir=".", iters=32, mixed_prec=False):
    ...
    with open(logdir + "/test.txt", "a") as file:
        file.write("Validation ETH3D: EPE %f, D1 %f\n" % (epe, d1))
    return {'eth3d-epe': epe, 'eth3d-d1': d1}


@torch.no_grad()
def validate_kitti(model, logdir=".", iters=32, mixed_prec=False):
    ...
    with open(logdir + "/test.txt", "a") as file:
        file.write(f"Validation KITTI: EPE {epe}, D1 {d1}, {format(1 / avg_runtime, '.2f')}-FPS ({format(avg_runtime, '.3f')}s)\n")
...
@torch.no_grad()
def validate_sceneflow(model, logdir=".", iters=32, mixed_prec=False):
    ...
    with open(logdir + "/test.txt", "a") as file:
        file.write("Validation SceneFlow: %f, %f\n" % (epe, d1))
...
@torch.no_grad()
def validate_middlebury(model, iters=32, split='F', logdir=".", mixed_prec=False):
    ...
    with open(logdir + "/test.txt", "a") as file:
        file.write(f"Validation Middlebury{split}: EPE {epe}, D1 {d1}")
