...
from evaluate_aeacv_stereo import *
from core.aeacv_stereo import AEACVStereo
...
def train(args):
    ...
    model = nn.DataParallel(AEACVStereo(args))
    print("Parameter Count: %d" % count_parameters(model))
    with open(args.logdir + "/model.txt", "w") as file:
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                if 'sceneflow' in args.train_datasets or 'sceneflow_mixdata' in args.train_datasets:
                    results = validate_sceneflow(model.module, iters=args.valid_iters, logdir=args.logdir)
                elif 'kitti' in args.train_datasets:
                    results = validate_kitti(model.module, iters=args.valid_iters, logdir=args.logdir)
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='aeacv-stereo', help="name your experiment")
    parser.add_argument('--restore_ckpt', default=None, help="load the weights from a specific checkpoint")
    parser.add_argument('--mixed_precision', default=True, action='store_true', help='use mixed precision')
    parser.add_argument('--logdir', default='./checkpoints/aeacv_sceneflow', help='the directory to save logs and checkpoints')
